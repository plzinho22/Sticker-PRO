...
(function () {
  ...
  var estado = {
    ...
    /* Fluxo de recuperação de senha via link do Supabase.
       O auth.js usa flowType 'implicit' (proposital: o link do
       e-mail costuma abrir em OUTRO navegador, onde o PKCE não
       teria o code_verifier). O link volta no fragmento
       #access_token=...&type=recovery e o evento PASSWORD_RECOVERY
       é disparado quando a sessão fica pronta. */
  ...
  /* ---------------------------------------------------------
     RETORNO DO LINK DE RECUPERAÇÃO — capturado no parse
     ---------------------------------------------------------
     Isto roda enquanto o script é lido, ANTES de o cliente do
     Supabase existir (auth.js cria o cliente sob demanda, na
     primeira chamada a Auth.aoMudar/sessao/token).

     É obrigatório capturar aqui: com detectSessionInUrl ligado,
     o SDK apaga o fragmento (#access_token=...&type=recovery)
     assim que transforma o retorno em sessão. Quem perguntar
     depois disso encontra a URL já limpa e conclui, errado, que
     não houve recuperação nenhuma.

     Cobre os dois formatos, sem depender do flowType:
       implicit -> #access_token=...&type=recovery
       PKCE     -> ?code=...
     --------------------------------------------------------- */
  var RETORNO_RECUPERACAO = (function () {
    try {
      ...
      var porCodigo = !!query.get('code');

      return porFragmento || porCodigo;
    } catch (e) {
      return false;
    }
  })();
  ...
  /* Baixa a figurinha como PNG de verdade no dispositivo. Reaproveita
     converterParaPng (mesma conversão usada em "Copiar") para garantir
     que o arquivo baixado seja sempre .png, mesmo que a imagem de
     origem esteja em outro formato. Não mexe em nada de copiarFigurinha. */
  function baixarFigurinha(url, botao) {
    var rotuloOriginal = botao.innerHTML;
    botao.disabled = true;
    botao.classList.add('baixando');
    botao.innerHTML =
      '<span class="fig-baixar-icone" aria-hidden="true">⏳</span>' +
      '<span class="fig-baixar-rotulo">Baixando…</span>';
    ...
    fetch(url, { cache: 'no-store' })
      ...
      .then(converterParaPng)
      .then(function (blobPng) {
        var linkUrl = URL.createObjectURL(blobPng);
        var a = document.createElement('a');
        a.href = linkUrl;
        a.download = 'figurinha-stickerpro.png';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        setTimeout(function () { URL.revokeObjectURL(linkUrl); }, 4000);

        botao.classList.remove('baixando');
        botao.classList.add('baixado');
        botao.innerHTML =
          '<span class="fig-baixar-icone" aria-hidden="true">✓</span>' +
          '<span class="fig-baixar-rotulo">Baixada!</span>';

        setTimeout(function () {
          botao.classList.remove('baixado');
          botao.disabled = false;
          botao.innerHTML = rotuloOriginal;
        }, 1800);
      })
      .catch(function (erro) {
        console.error('[Baixar figurinha] falha:', erro);
        botao.classList.remove('baixando');
        botao.disabled = false;
        botao.innerHTML = rotuloOriginal;
        avisarFigura(botao, 'Não foi possível baixar a figurinha. Tente novamente.');
      });
  }

  /* HTML de uma figura, com os botões de copiar e baixar embutidos.
     Usado nos três pontos do arquivo que montam a galeria, para não
     duplicar os mesmos botões em três lugares diferentes. */
    ...
    return (
      '<figure class="fig">' +
        ...
        '<div class="fig-acoes">' +
          ...
          '</button>' +
          '<button type="button" class="fig-baixar" data-url="' + u + '">' +
            '<span class="fig-baixar-icone" aria-hidden="true">⬇</span>' +
            '<span class="fig-baixar-rotulo">Baixar</span>' +
          '</button>' +
        '</div>' +
      '</figure>'
    );
  ...
  /* Mesmo padrão de listener delegado, só que para o botão de baixar.
     Função separada de propósito: não toca em nada de iniciarCopiarFigurinhas. */
  function iniciarBaixarFigurinhas() {
    document.addEventListener('click', function (ev) {
      var botao = ev.target && ev.target.closest && ev.target.closest('.fig-baixar');
      if (!botao || botao.disabled) return;
      var url = botao.getAttribute('data-url');
      if (url) baixarFigurinha(url, botao);
    });
  }
...
function listarPastaStorage(pasta, token, offset) {
  ...
  function entrarComSessao(email) {
    /* Uma sessão de recuperação é uma sessão válida, mas NÃO é um
       login: a pessoa ainda precisa definir a senha. Sem esta
       guarda, o app abre a área de membros por cima da tela
       "Escolha uma nova senha", a pessoa entra achando que
       terminou e a senha nunca chega a ser gravada.

       iniciarRedefinicao() zera os dois sinalizadores logo antes
       de chamar entrarComSessao() no sucesso do updateUser, então
       o acesso após criar a senha segue normalmente. */
    if (estado.recuperacaoSenha || estado.aguardandoRecuperacao) {
  ...
  /* RETORNO_RECUPERACAO vem primeiro porque é a única fonte
     confiável depois que o SDK limpa a URL. As duas leituras ao
     vivo continuam valendo para o caso de o SDK ainda não ter
     processado o retorno. */
  function temRecuperacao() {
    return RETORNO_RECUPERACAO ||
           temCodigoDeRecuperacao() ||
           temTokenDeRecuperacao();
  }
   ...
   */
  /*
   * Garante a sessão do fluxo de recuperação sem trocar o ?code= manualmente.
   *
   * O auth.js já cria o cliente Supabase principal com detectSessionInUrl=true
   * + PKCE. Quando o link de recuperação abre, o Supabase processa o ?code=
   * e grava a sessão. Aqui apenas aguardamos essa sessão ficar disponível.
   *
   * Isso evita usar um segundo cliente Supabase para chamar
   * exchangeCodeForSession(), o que poderia tentar consumir o mesmo código
   * duas vezes.
   */
      ...
      return Promise.resolve(Auth.sessao())
        .then(function (usuario) {
          ...
          if (tentativas >= maxTentativas) {
            throw new Error(
              'A sessão para criar sua senha não foi iniciada. Peça um novo link de recuperação.'
            );
          }
